...
def preprocess_data(df, is_test_data=False):
    ...
    # Check if we need to calculate age from birthdate
    if 'Big_Age' not in X.columns and 'Big Birthdate' in X.columns:
        print("Calculating ages from birthdates...")
        # Convert birthdates to datetime
        X['Big Birthdate'] = pd.to_datetime(X['Big Birthdate'], errors='coerce')
        X['Little Birthdate'] = pd.to_datetime(X['Little Birthdate'], errors='coerce')
        
        # Calculate ages
        today = pd.Timestamp.now()
        X['Big_Age'] = (today - X['Big Birthdate']).dt.total_seconds() / (365.25 * 24 * 60 * 60)
        X['Little_Age'] = (today - X['Little Birthdate']).dt.total_seconds() / (365.25 * 24 * 60 * 60)
    ...
    cols_to_drop = [
        ...
        'Match Length',  # Target variable (may not exist in test data)
    ...
    # Get target variable if it exists (won't exist in test data)
    if 'Match Length' in df.columns and not is_test_data:
        y = df['Match Length'].copy()
    else:
        # For test data, create a dummy target variable
        y = pd.Series(np.zeros(len(df)))
    
    # Age-related feature engineering
    print("Creating age-related features...")
    
    # Calculate age difference (with error handling)
    if 'Big_Age' in X.columns and 'Little_Age' in X.columns:
        ...
        if max_age_diff > 0:
            ...
            X['Age_Compatibility_Score'] = 1.0
    else:
        print("Warning: Age columns not found, using default values")
        X['Age_Difference'] = 15.0  # Average age difference from training data
        X['Age_Compatibility_Score'] = 0.5  # Middle value
...
def predict_on_test_data(test_df, models, cluster_centers, output_path='predictions.xlsx'):
    """
    Make predictions on test data and save to Excel in the required format
    """
    print("\nMaking predictions on test data...")
    
    # Preprocess the test data (similar to training data preprocessing)
    X_test, _ = preprocess_data(test_df, is_test_data=True)
    
    # Get the Match ID column for output
    match_ids = test_df['Match ID 18Char'].values
    
    # Assign each test sample to a cluster
    # Select the same features used for clustering in training
    clustering_features = [
        'Program Type_Community', 'Program Type_School', 'Program_Maturity_Years', 
        'Age_Difference', 'Gender_Match', 'Race_Ethnicity_Match',
        'Big_Age', 'Little_Age', 'Demographic_Similarity_Score',
    ...
    # Filter existing columns that are in the dataset
    clustering_features = [col for col in clustering_features if col in X_test.columns]
    
    # Standardize features for clustering
    scaler = StandardScaler()
    X_cluster = scaler.fit_transform(X_test[clustering_features])
    
    # Instead of using KMeans predict, manually assign to nearest centroid
    ...
    for sample in X_cluster:
        ...
        test_clusters.append(closest_cluster)
    
    # Make predictions
    predictions = []
    
    # Define a fallback model in case a cluster doesn't have a specific model
    fallback_model = None
    if models:
    ...
    if fallback_model is not None:
        model_features = fallback_model.feature_names_in_
        print(f"Model was trained on {len(model_features)} features")
        
        # Align test data features with model features
        print("Aligning test data features with model features...")
        X_aligned = pd.DataFrame(index=X_test.index)
        
        # Add all model features, filling with 0 for missing ones
        for feature in model_features:
            if feature in X_test.columns:
                X_aligned[feature] = X_test[feature]
            else:
                X_aligned[feature] = 0
        
        # Now X_aligned has exactly the same features as the model expects
        print(f"Aligned features: {X_aligned.shape[1]}")
    else:
        X_aligned = X_test
        print("Warning: No model available to extract features from")
    
    for i, cluster_id in enumerate(test_clusters):
        # Get the sample
        X_sample = X_aligned.iloc[[i]]
        ...
        if model is not None:
            ...
            # If no model is available, use the average match length
            predictions.append(24.0)  # Average match length from training data
    
    # Create output DataFrame
    output_df = pd.DataFrame({
        'RowID': range(1, len(match_ids) + 1),
        'MatchID18Char': match_ids,
        'PredictedMatchLength': predictions,
        'YourTeamID': 'G15'
    })
...
def main():
    # Check if we're in prediction mode (test data) or training mode
    import sys
    prediction_mode = len(sys.argv) > 1 and sys.argv[1] == 'predict'
    
    if prediction_mode:
        print("Running in prediction mode on test data...")
        
        # Load the enhanced test data
        try:
            # Update path to use the enhanced test data from the submission directory
            test_df = pd.read_excel('/Users/dameesh/Desktop/MinMUDAC2025/submission/enhanced_test_data.xlsx')
            print("Enhanced test data loaded successfully")
            
            # Load the trained models
            import pickle
            with open('cluster_qrf_models.pkl', 'rb') as f:
            ...
            print("Loaded cluster centers")
            
            # Make predictions and save to Excel
            predict_on_test_data(test_df, models, cluster_centers, 'G15_predictions.xlsx')
        ...
        print("Loading data...")
        df = pd.read_excel('/Users/dameesh/Desktop/MinMUDAC2025/submission/enhanced_data.xlsx')
        ...
        # Save cluster centers for later prediction
        import pickle
        kmeans = KMeans(n_clusters=n_clusters, random_state=42, n_init=10)
        clustering_features = [
            'Program Type_Community', 'Program Type_School', 'Program_Maturity_Years', 
        ...
        print("Saved cluster centers for future prediction")
